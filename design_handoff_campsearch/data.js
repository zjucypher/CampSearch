...
window.CS_DATA = (() => {
  ...
  // Build a 6-row x 7-col calendar for July 2026
  // Jul 1 is a Wednesday in 2026
  const calendar = {
    month: "July 2026",
    days: (() => {
      const out = [];
      // June leading days (Sun=28, Mon=29, Tue=30)
      [28, 29, 30].forEach(d => out.push({ d, out: true }));
      for (let d = 1; d <= 31; d++) {
        out.push({
          d,
          avail: [5, 6, 11, 17, 18, 22, 28].includes(d),
          range: d >= 18 && d <= 20 ? (d === 18 ? "start" : d === 20 ? "end" : "mid") : null,
        });
      }
      // Aug leading days to fill the grid (need to reach 42 cells)
      let i = 1;
  ...
  // 8x5 grid of sites for the campground detail
  const sites = Array.from({ length: 40 }, (_, i) => {
    const num = i + 1;
    const status =
      [3, 7, 9, 12, 18, 21, 25, 30, 36].includes(num) ? "available" :
      [5, 14, 15, 22, 38].includes(num) ? "watched" :
      "taken";
    return { num, status, type: num % 6 === 0 ? "RV" : "Tent" };
  });

  return { campgrounds, alerts, history, calendar, sites };
})();
